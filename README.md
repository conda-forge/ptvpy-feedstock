...
Once the `conda-forge` channel has been enabled, `ptvpy` can be installed with `conda`:
...
It is possible to list all of the versions of `ptvpy` available on your platform with `conda`:
...
```
mamba search ptvpy --channel conda-forge
```
...
mamba repoquery search ptvpy --channel conda-forge

# List packages depending on `ptvpy`:
mamba repoquery whoneeds ptvpy --channel conda-forge

# List dependencies of `ptvpy`:
mamba repoquery depends ptvpy --channel conda-forge
```
